...
class FP_DivergenceExplorer_ranking:
    ...
    def computeDivergenceItemsets(
        ...
        if D_OUTCOME in metrics:
            ...
            if "d_fnr_w" in metrics:
                alfaFN = (fm_df["tp"] + fm_df["fn"]) / infoRoot["support_count"].values[
                    0
                ]
                fm_df["d_fnr_w"] = alfaFN * fm_df["d_fnr"]
            if "d_fpr_w" in metrics:
                alfaFP = (fm_df["tn"] + fm_df["fp"]) / infoRoot["support_count"].values[
                    0
                ]
    ...
    def fpgrowth_divergence_metrics(
        self,
        df_data,
        df_targets,  #df_confusion_matrix for classification purposes
        min_support=0.5,
        use_colnames=False,
        verbose=0,
        cols_orderTP=["tn", "fp", "fn", "tp"],
        ...
        from .utils_FPgrowth_all import fpgrowth_cm

        # For classification purposes
        # df_data are the one-hot-encoding of the data only
        # df_targets corresponds to the confusion matrix dataframe
        # e.g.
        # tn  fp  fn  tp
        # 0   1   0   0   0
        # 1   0   0   1   0
        ...
        fp["support_count"] = (fp["support"] * len(df_data)).round()
    ...
    def getFrequentPatternDivergence(
        ...
        # df_FP_divergence = self.t_test_FP(min_support, metrics=metrics)

        return df_FP_divergence

    def mean_var_beta_distribution(self, FP_df, metric):  # Updated
    ...
    def t_test_FP(self, FP_df, metrics=["d_fpr", "d_fnr", "d_accuracy"]):  # Updated
        for metric in metrics:
            ...
            FP_df = self.mean_var_beta_distribution(FP_df, metric)

            mean_col, var_col = f"mean_beta_{c_metric}", f"var_beta_{c_metric}"
            mean_d, var_d = FP_df.loc[FP_df.itemsets == frozenset()][
                [mean_col, var_col]
            ].values[0]
            FP_df[f"t_value_{c_metric}"] = _compute_t_test(
                FP_df[[mean_col, var_col]], mean_col, var_col, mean_d, var_d
            )
            FP_df.drop(
    ...
    def statistical_significance_outcome(
        ...
        # infoRoot_dict = getInfoRoot(fm_df).T.to_dict()[0]
