...
def averageOutcome(df_score_res, score_col, count_col):
...
def getPerformanceMetric(
    ...
    if target_classification:
        ...
        return average_outcome_df(data_sel, target_name)


def average_outcome_df(data_sel, target_name):
    outcome_values = data_sel[target_name].values
    return np.sum(outcome_values) / len(outcome_values)


def average_outcome_np(outcome_values):
    return np.sum(outcome_values) / len(outcome_values)


def sum_len_outcome_np(outcome_values):
    return {"SUM": np.sum(outcome_values), "LEN": len(outcome_values)}
...
def getPerformanceMetric_np(
    ...
    if metric == D_OUTCOME:
        return np.sum(conf_m) / len(conf_m)
    else:
        if metric == "d_fpr":
            num_v = ["fp"]
            den_v = ["fp", "tn"]
