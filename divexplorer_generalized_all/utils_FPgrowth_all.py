...
def setup_fptree(df, min_support, df_targets):
    ...
    tree = FPTree(rank, cm_root=np.zeros_like(df_targets[0]))
    for i in range(num_itemsets):
        ...
        tree.insert_itemset(itemset, cm_i=df_targets[i].copy())
...
def _get_df_top_k(
    ...
    """
    For the average score we also need the len_dataset
    """
...
class FPTree(object):
    def __init__(self, rank=None, cm_root=None):
        self.root = FPNode_CM(None, target_matrix=cm_root)
        self.nodes = collections.defaultdict(list)
        self.cond_items = []
        self.rank = rank
        self.root_cm_root = cm_root
    ...
    def conditional_tree(self, cond_item, minsup):
        ...
        cond_tree = FPTree(rank, cm_root=np.zeros_like(self.root_cm_root))
        for idx, branch in enumerate(branches):
            ...
            cond_tree.insert_itemset(
                ...
                cm_i=self.nodes[cond_item][idx].target_matrix.copy(),  ## TODO .copy(),
            )
    ...
    def insert_itemset(self, itemset, count=1, cm_i=None):
        ...
        for item in itemset:
            if item in node.children:
                ...
                child.count += count
                child.target_matrix += cm_i.copy()  ## TODO .copy(),
                node = child
                index += 1
            else:
                break
        ...
        for item in itemset[index:]:
            child_node = FPNode_CM(
                item, count, node, target_matrix=cm_i.copy()
            )  ## TODO .copy(),
...
class FPNode_CM(object):
    def __init__(
        self, item, count=0, parent=None, target_matrix=None
...
def fpgrowth_cm(
    ...
    return generate_itemsets(
        ...
        row_root=row_root,
        len_dataset=len_dataset,  # to compute the support count we need the len of the dataset
    )
